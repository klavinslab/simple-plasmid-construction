...
class Protocol
  ...
  def prepare_electroporator
    show do
      ...
      check "Retrieve and label #{operations.running.length} 1.5 mL tubes with the following ids: #{operations.running.collect { |op| op.output(OUTPUT).item.id.to_s }.join(',')} "
  ...
  def determine_replacements
    operations.running.each do |op|
      ...
      next unless op.input(CELLS).collection.empty? || (debug && DEBUG_WITH_REPLACEMENT)
      ...
      batches_of_cells = all_batches.reject { |b| b == old_batch }.sort(&:num_samples) # debug specific rejection to force replacement
