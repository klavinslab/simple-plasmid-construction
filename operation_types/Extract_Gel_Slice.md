...
Extracts a gel fragment from a gel.
...
class Protocol
  ...
  MIN_WEIGHT = 0.0
  MAX_WEIGHT = 10.0
  CORRECT = %w[y n].freeze # values for debug
  ...
  def main
    ...
    choice = show do
      title 'What Next?'
      select %w[Yes No], var: 'choice', label: 'Would you like to purify the gel slices immediately?', default: 0
    end
  ...
  def cut_fragments(grouped_ops)
    show do
      ...
      note "Take out #{grouped_ops.length} 1.5 mL tubes and label accordingly: #{grouped_ops.map { |op| op.output('Fragment').item.to_s }.to_sentence}"
      note 'Now, cut out the bands and place them into the 1.5 mL tubes according to the following table:'
      table grouped_ops.start_table
                       .custom_column(heading: 'Gel ID') { |op| op.input(FRAGMENT).item.to_s }
                       .custom_column(heading: 'Row') { |op| op.input(FRAGMENT).row + 1 }
                       .custom_column(heading: 'Column', checkable: true) { |op| op.input(FRAGMENT).column + 1 }
                       .custom_column(heading: '1.5 mL Tube ID') { |op| op.output(FRAGMENT_OUT).item.to_s }
  ...
  def clean_up(gel, gels)
    show do
      ...
      check 'Dispose gloves after leaving the room.' if gel == gels.last
    end
  end
end
