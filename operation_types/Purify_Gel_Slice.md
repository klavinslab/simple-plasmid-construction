...
Purifies an input gel slice that has been extracted.
...
class Protocol
  ...
  def get_choices
    if operations.any? { |op| op.output_data('Fragment', :concentration) < 10 }
      choices = show do
        ...
        operations.select { |op| op.output_data('Fragment', :concentration) < 10 }.each do |op|
          select %w[Yes No], var: "d#{op.output('Fragment').item.id}", label: "Discard Fragment Stock #{op.output('Fragment').item.id}", default: 1
        end
      end
    end
    choices # return
  ...
  def move_gel_slices_to_tubes
    if operations.any? { |op| op.temporary[:total_volume].between?(1500, 2000) }
      show do
  ...
  def distribute_gels
    if operations.any? { |op| op.temporary[:is_divided] }
      show do
  ...
  def add_isopropanol
    if operations.any? { |op| op.temporary[:iso_volume] > 0 }
      show do
  ...
  def discard_fragment_stocks(choices)
    if !choices.nil? && choices.any? { |_key, val| val == 'Yes' }
      show do
